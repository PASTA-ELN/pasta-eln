...
class TestDataHierarchyLookupIriAction(object):
  ...
  def test_lookup_iri_action_initialize_should_succeed(self, mocker):
    ...
    mock_icon = mocker.patch('pasta_eln.GUI.data_hierarchy.lookup_iri_action.icon')
    ...
    mock_terminology_lookup_dialog = mocker.patch(
      'pasta_eln.GUI.data_hierarchy.lookup_iri_action.TerminologyLookupDialog', return_value=mock_dialog)
    action = LookupIriAction(lookup_term="default", parent_line_edit=mock_parent, cell_index=mock_cell_index)
    mock_base_init.assert_called_once_with(icon=mock_icon.return_value, text="Lookup IRI online", parent=mock_parent)
    mock_base_triggered_connect.assert_called_once_with(action.show_terminology_lookup_dialog)
    mock_get_logger.assert_called_once_with('pasta_eln.GUI.data_hierarchy.lookup_iri_action.LookupIriAction')
  ...
  def test_lookup_iris_invoke_should_show_terminology_lookup_dialog(self, mocker, lookup_iri_action: lookup_iri_action):
    mocker.patch.object(QAction, 'triggered', create=True)
  ...
  def test_terminology_lookup_accepted_callback_should_do_as_expected(self, mocker,
