...
def delegate_paint_common(mocker, delegate: Union[delete_delegate, reorder_delegate], button_icon):
  ...
  mocker.patch.object(QPushButton, "__new__", lambda x: mock_push_button)
  mocker.patch.object(QStyleOptionButton, "__new__", lambda x: mock_button_option)
  mocker.patch.object(QSize, "__new__", lambda x, y, z: mock_icon_size)
...
def delegate_editor_method_common(delegate: Union[delete_delegate, reorder_delegate], mocker):
  mock_painter = mocker.patch("PySide6.QtGui.QPainter")
  mock_option = mocker.patch("PySide6.QtWidgets.QStyleOptionViewItem")
  mock_index = mocker.patch("PySide6.QtCore.QModelIndex")
  assert delegate.createEditor(mock_painter, mock_option, mock_index) is None, "create editor should return None"


def delegate_editor_event_common(delegate: Union[delete_delegate, reorder_delegate], mocker, is_click_within_bounds):
  mock_event = mocker.patch("PySide6.QtCore.QEvent")
  mock_model = mocker.patch("PySide6.QtCore.QAbstractItemModel")
  mock_index = mocker.patch("PySide6.QtCore.QModelIndex")
  mock_option = mocker.patch("PySide6.QtWidgets.QStyleOptionViewItem")
  mocker.patch.object(mock_index, "row", mocker.MagicMock(return_value=2))
  emit_spy = None
  ...
  assert delegate.editorEvent(mock_event, mock_model, mock_option,
