...
class ClickableFrame(QPushButton):
  ...
  def __init__(self, docID: str):
    ...
    super().__init__()
    self.docID = docID
    self.setFixedSize(imgSize, imgSize)
  ...
  def onGetData(self, doc:dict[str,Any]) -> None:
    ...
    if 'id' in doc and doc['id'] in self.data:
      ...
      if image.startswith('<?xml version="1.0"'):
        ...
        button.setFixedSize(imgSize, imgSize)
        ...
        if svgWidth == 0 or svgHeight == 0:                                           # Avoid division by zero
          ...
          newWidth = imgSize - 4
          newHeight = int(float(svgHeight) / float(svgWidth) * (imgSize - 4))
        ...
        try:
          ...
          if imageW.loadFromData(byteArr, format=fmt):
            pixmap = QPixmap.fromImage(imageW).scaled(imgSize,imgSize,Qt.KeepAspectRatio,Qt.SmoothTransformation)# type: ignore[attr-defined]
            button = ClickableFrame(doc['id'])
