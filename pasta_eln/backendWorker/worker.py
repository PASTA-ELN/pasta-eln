...
class Task(Enum):
    ...
    EXTRACTOR_TEST = (
        10,
        "Testing extractor:",
    )  # keys: fileName, style, recipe, saveFig
    ...
    )  # keys: projID, docTypes,repositories,metadata,uploadZenodo
...
class BackendWorker(QObject):
    ...
    def initialize(self, configuration: dict[str, Any], projectGroupName: str) -> None:
        ...
        docTypesTitlesIcons = {
            k: {"title": v} for k, v in self.backend.db.dataHierarchy("", "title")
        }
        ...
        for docType in docTypesTitlesIcons:
            self.beSendDataHierarchyNode.emit(
                docType, self.backend.db.dataHierarchy(docType, "meta")
            )
    ...
    def returnDataHierarchyRow(self, docType: str) -> None:
        ...
        if self.backend is not None:
            self.beSendDataHierarchyAll.emit(
                self.backend.db.dataHierarchy(docType, "*")
            )
    ...
    def returnHierarchy(self, projID: str, showAll: bool) -> None:
        ...
        """
        if (
            self.backend is not None and projID
        ):  # TODO: during test_13 for some reason the projID is empty, not sure why
    ...
    def returnTaskReport(self, task: Task, data: dict[str, Any]) -> None:
        ...
        if self.backend is None:
            return
        if task is Task.EXTRACTOR_TEST and set(data.keys()) == {
            "fileName",
            "style",
            "recipe",
            "saveFig",
        }:
            report, image = self.backend.testExtractor(
                data["fileName"],
                style={"main": data["recipe"]},
                outputStyle=data["style"],
                saveFig=data["saveFig"],
            )
        ...
        elif task is Task.ADD_DOC and set(data.keys()) == {
            "hierStack",
            "docType",
            "doc",
        }:
            if data["hierStack"]:
                ...
                self.backend.cwd = Path(
                    self.backend.db.getDoc(parentID)["branch"][0]["path"]
                )
            else:  # project
            ...
            if (
                data["newProjID"]
                and "branch" in data["doc"]
                and len(data["doc"]["branch"][0]["stack"]) > 0
                and data["doc"]["branch"][0]["stack"][0] != data["newProjID"][0]
            ):  # only if project changed
                if data["doc"]["branch"][0]["path"] is None:
                    newPath = ""
                else:
                    oldPath = self.backend.basePath / data["doc"]["branch"][0]["path"]
                    parentPath = self.backend.db.getDoc(data["newProjID"][0])["branch"][
                        0
                    ]["path"]
                    newPath = f"{parentPath}/{oldPath.name}"
                data["doc"]["branch"][0] = {
                    "stack": []
                    if data["newProjID"][0] == "NONE"
                    else [data["newProjID"][0]],
                    "path": newPath or None,
                    "child": 9999,
                    "show": [True, True],
                }
            ...
            self.beSendDoc.emit(
                self.backend.db.getDoc(data["doc"]["id"])
            )  # send updated doc back to GUI

        elif task is Task.MOVE_LEAVES and set(data.keys()) == {
            "docID",
            "stackOld",
            "stackNew",
            "childOld",
            ...
            if len(branchOldList) != 1:
                branchOldList = [
                    i for i in branchOldList if i["child"] == data["childOld"]
                ]
            if len(branchOldList) != 1:
                logging.error(
                    "Cannot move leaves: %s has no branch with stack %s",
                    doc["id"],
                    data["stackOld"],
                    exc_info=True,
                )
            ...
            if branchOld["path"] is not None and not branchOld["path"].startswith(
                "http"
            ):
                parentDir = Path(
                    self.backend.db.getDoc(data["stackNew"][-1])["branch"][0]["path"]
                )
                if doc["type"][0][0] == "x":
                    dirNameNew = createDirName(
                        doc, data["childNew"], parentDir
                    )  # create path name: do not create directory on disk yet
                else:
                    dirNameNew = Path(branchOld["path"]).name  # use old name
                pathNew = f"{parentDir}/{dirNameNew}"
            else:
                pathNew = branchOld["path"]
            siblingsNew = self.backend.db.getView(
                "viewHierarchy/viewHierarchy", startKey="/".join(data["stackNew"])
            )  # sorted by docID
            siblingsNew = [
                i
                for i in siblingsNew
                if len(i["key"].split("/")) == len(data["stackNew"]) + 1
            ]
            childNums = [
                f"{i['value'][0]:05d}{i['id']}{idx}"
                for idx, i in enumerate(siblingsNew)
            ]
            siblingsNew = [
                x for _, x in sorted(zip(childNums, siblingsNew))
            ]  # sorted by childNum 1st and docID 2nd
            # --- CHANGE ----
            # change new siblings
            if verbose:
                print(
            ...
            for idx, line in reversed(list(enumerate(siblingsNew))):
                shift = (
                    1 if idx >= data["childNew"] else 0
                )  # shift those before the insertion point by 0 and those after by 1
                if (
                    line["id"] == data["docID"] or line["value"][0] == idx + shift
                ):  # ignore this id & those that are correct already
                    continue
                if verbose:
                    print(f"  {line['id']}: move: {idx} {shift}")
                self.backend.db.updateBranch(
                    docID=line["id"], branch=line["value"][4], child=idx + shift
                )
            if verbose:
                siblingsNew = self.backend.db.getView(
                    "viewHierarchy/viewHierarchy", startKey="/".join(data["stackNew"])
            ...
            if verbose:
                print(
                    f"  manual move {data['childOld']} -> {data['childNew']}: {data['docID']}"
                )
            self.backend.db.updateBranch(
                docID=data["docID"],
                branch=-99,
                ...
                path=pathNew,
                child=data["childNew"],
                stackOld=data["stackOld"] + [data["docID"]],
            )
            # change old siblings
            siblingsOld = self.backend.db.getView(
                "viewHierarchy/viewHierarchy", startKey="/".join(data["stackOld"])
            )  # sorted by docID
            siblingsOld = [
                i
                for i in siblingsOld
                if len(i["key"].split("/")) == len(data["stackOld"]) + 1
            ]
            childNums = [
                f"{i['value'][0]:05d}{i['id']}{idx}"
                for idx, i in enumerate(siblingsOld)
            ]
            siblingsOld = [
                x for _, x in sorted(zip(childNums, siblingsOld))
            ]  # sorted by childNum 1st and docID 2nd
            if verbose:
            ...
            for idx, line in enumerate(siblingsOld):
                if (
                    line["value"][0] == idx
                ):  # ignore id in question and those that are correct already
                    continue
                if verbose:
                    print(f"  {line['id']}: move: {idx}")
                self.backend.db.updateBranch(
                    docID=line["id"], branch=line["value"][4], child=idx
                )
            if verbose:
                siblingsOld = self.backend.db.getView(
                    "viewHierarchy/viewHierarchy", startKey="/".join(data["stackOld"])
            ...
            for item in data["items"]:
                ...
                while targetName.exists():
                    targetName = targetFolder / (
                        itemPath.stem + f"_{idx}" + itemPath.suffix
                    )
                    idx += 1
                if itemPath.is_dir():
            ...
            reply = ""
            for _ in range(2):  # scan twice: convert, extract
                reply += self.backend.scanProject(
                    None, data["docID"], targetFolder.relative_to(self.backend.basePath)
                )
            ...
            if (
                "branch" in doc
                and len(doc["branch"]) > 0
                and "path" in doc["branch"][0]
            ):
            ...
            children = self.backend.db.getView(
                "viewHierarchy/viewHierarchy",
                startKey="/".join(doc["branch"][0]["stack"] + [data["docID"]]),
            )
            for docID in {
                line["id"] for line in children if line["id"] != data["docID"]
            }:
        ...
        elif task is Task.EXPORT_ELN and set(data.keys()) == {
            "fileName",
            "projID",
            "docTypes",
        }:
            report = exportELN(
                self.backend, [data["projID"]], data["fileName"], data["docTypes"]
            )
            self.beSendTaskReport.emit(task, report, "", "")

        elif task is Task.IMPORT_ELN and set(data.keys()) == {"fileName", "projID"}:
            report, statistics = importELN(
                self.backend, data["fileName"], data["projID"]
            )
            self.beSendTaskReport.emit(
                task, f"{report}\n{json.dumps(statistics, indent=2)}", "", ""
            )

        elif task in (Task.SEND_ELAB, Task.GET_ELAB, Task.SMART_ELAB) and set(
            data.keys()
        ) == {"projGroup"}:
            if "ERROR" in self.backend.checkDB(minimal=True):
                self.beSendTaskReport.emit(
                    task,
                    "ERRORs are present in your database. Fix them before uploading",
                    "",
                    "",
                )
            else:
                try:
                    ...
                    if (
                        hasattr(sync, "api") and sync.api.url
                    ):  # if hostname and api-key given
                        ...
                        msg = ", ".join(
                            [f"{MERGE_LABELS[k]}: {v}" for k, v in statsCount.items()]
                        )
                        self.beSendTaskReport.emit(
                            task,
                            f"Success: Synchronized with server. Items per action: {msg}",
                            "",
                            "",
                        )
                    else:  # if not given
                        self.beSendTaskReport.emit(
                            task,
                            "ERROR: Please specify a server address and API-key in the Configuration",
                            "",
                            "",
                        )
                    ...
                    self.beSendTaskReport.emit(
                        task, f"ERROR: Connection error to elabFTW server\n{e}", "", ""
                    )

        elif task is Task.SEND_TBL_COLUMN and set(data.keys()) == {
            "docType",
            "newList",
        }:
            self.backend.db.dataHierarchyChangeView(data["docType"], data["newList"])

        elif task is Task.SEND_REPOSITORY and set(data.keys()) == {
            "projID",
            "docTypes",
            "repositories",
            "metadata",
            ...
            if data["uploadZenodo"]:  # Zenodo
                clientZ = ZenodoClient(
                    repositories["zenodo"]["url"], repositories["zenodo"]["key"]
                )
                metadataZ = clientZ.prepareMetadata(data["metadata"])
                res = clientZ.uploadRepository(metadataZ, tempELN)
            else:  # Dataverse
                clientD = DataverseClient(
                    repositories["dataverse"]["url"],
                    repositories["dataverse"]["key"],
            ...
            if res[0]:
                ...
                docProject[".repository_upload"] = (
                    f"{datetime.now().strftime('%Y-%m-%d')} {res[1]}"
                )
            ...
            for docID in data["docIDs"]:
                ...
                if doc["branch"][0]["path"] is not None:
                    ...
                    if doc["type"][0] == oldDocType[0]:
                        ...
                        self.backend.addData(
                            "/".join(doc["type"]), doc, doc["branch"][0]["stack"]
                        )
            ...
            if doc["branch"][0]["path"] is None:
                QMessageBox.critical(
                    None, "ERROR", "Cannot open file that is only in the database"
                )
            else:
            ...
            logging.error(
                "Got task, which I do not understand %s %s",
                task,
                data.keys(),
                exc_info=True,
            )
