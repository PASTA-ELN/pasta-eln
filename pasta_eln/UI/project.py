...
class Project(QWidget):
    ...
    def projHeader(self) -> None:
        ...
        hidden, menuTextHidden = (
            ("     \U0001f441", "Mark project as shown")
            if [b for b in self.docProj["branch"] if False in b["show"]]
            else ("", "Mark project as hidden")
        )
        ...
        # buttons in top line
        self.btnAddSubfolder = TextButton(
            "Add subfolder", self, [Command.ADD_CHILD], topLineL
        )
        ...
        self.actHideDetail = Action(
            "Hide project details", self, [Command.SHOW_PROJ_DETAILS], visibilityMenu
        )
        menuTextItems = "Hide hidden items" if self.showAll else "Show hidden items"
        minimizeItems = (
            "Show all item details" if self.showDetailsAll else "Hide all item details"
        )
        Action(menuTextItems, self, [Command.HIDE_SHOW_ITEMS], visibilityMenu)
        Action(menuTextHidden, self, [Command.HIDE], visibilityMenu)
        self.actionFoldAll = Action(
            minimizeItems, self, [Command.SHOW_DETAILS], visibilityMenu
        )
        ...
        for docType, value in self.comm.docTypesTitles.items():
            if docType[0] != "x":
                ...
                Action(
                    f"list of {value['title'].lower()}",
                    self,
                    [Command.SHOW_TABLE, docType],
                    moreMenu,
                    icon=icon,
                )
        Action(
        ...
        self.allDetails.setMarkdown(
            doc2markdown(
                self.docProj, DO_NOT_RENDER, self.comm.dataHierarchyNodes["x0"], self
            )
        )
        ...
        self.allDetails.setStyleSheet(
            f"border: none; padding: 0px; {bgColor} {fgColor}"
        )
    ...
    def commentResize(self, _: Any) -> None:
        ...
        self.allDetails.setHorizontalScrollBarPolicy(
    ...
    def paint(self) -> None:
        ...
        if (
            ...
            and self.hierarchy.name == "__ERROR_in_getHierarchy__"
        ):
            showMessage(
                self,
                "Error",
                "There is an error in the project hierarchy: a parent of a node is incorrect.",
                "Critical",
            )
            return
        try:
            ...
            self.tree = TreeView(
                self, self.comm, QStandardItemModel()
            )  # if hierarchy is None, create empty tree
        ...
        if (
            self.hierarchy is not None
            and len(self.hierarchy.children) > 0
            and self.btnAddSubfolder is not None
        ):
        ...
        self.tree.collapsed.connect(
            lambda index: self.actionExpandCollapse(index, False)
        )
    ...
    def actionExpandCollapse(self, index: QModelIndex, flag: bool) -> None:
        ...
        self.model.itemFromIndex(index).setData(
            {**meta, **{"gui": gui}}, self.META_ROLE
        )
    ...
    def execute(self, command: list[Any]) -> None:
        ...
        if command[0] is Command.EDIT:
            ...
            if oldPath.is_dir():
                newPath = self.comm.basePath / createDirName(
                    self.docProj, 0, self.comm.basePath
                )
                if oldPath != newPath:
            ...
            self.comm.uiRequestTask.emit(
                Task.SET_GUI, {"docID": self.projID, "gui": self.docProj["gui"]}
            )
            ...
            def recursiveRowIteration(index: QModelIndex) -> None:
                for subRow in range(self.tree.model().rowCount(index)):  # type: ignore[union-attr]
                    ...
                    self.comm.uiRequestTask.emit(
                        Task.SET_GUI, {"docID": docID, "gui": gui}
                    )
    ...
    def modelChanged(self, item: QStandardItem) -> None:
        ...
        # compare
        logging.debug(
            "Change project: docID %s | old stack %s child %i | new stack %s child %i",
            docID,
            str(stackOld),
            childOld,
            str(stackNew),
        ...
        if (
            stackOld == stackNew and childOld == childNew
        ):  # nothing changed, just redraw
            return
        self.comm.uiRequestTask.emit(
            Task.MOVE_LEAVES,
            {
        ...
        item.setData(
            item.data()
            | {"hierStack": "/".join(stackNew + [docID]), "childNum": childNew}
        )

        def updateChildrenStacks(
            parentItem: QStandardItem, parentStack: list[str]
        ) -> None:
            ...
            for row in range(parentItem.rowCount()):
                ...
                childItem.setData(
                    metaChild | {"hierStack": "/".join(newStack), "childNum": row},
                    self.META_ROLE,
                )
    ...
    def iterateTree(self, nodeHier: Node) -> QStandardItem:
        ...
        nodeTree.setData(
            {
                "hierStack": hierStack,
                "docType": nodeHier.docType,
                "gui": gui,
                "childNum": nodeHier.childNum,
            },
            self.META_ROLE,
        )
        if nodeHier.id[0] == "x":
